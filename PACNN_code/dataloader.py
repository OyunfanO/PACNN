        ...
        while watchdog.is_alive():
            try:
                #r = index_queue.get(timeout=MP_STATUS_CHECK_INTERVAL)
                 r = index_queue.get(timeout=utils.MP_STATUS_CHECK_INTERVAL)
            except queue.Empty:
                continue
            if r is None:
...
class MSDataLoader(DataLoader):
    ...
    def __init__(self, args, dataset, batch_size=1, shuffle=False, sampler=None, batch_sampler=None,
                 num_workers=0, collate_fn=_utils.collate.default_collate, pin_memory=False, drop_last=False,
                 timeout=0, worker_init_fn=None):
