...
class QConv(nn.Module):
    def __init__(self, inChannels, outChannels, kSize=3, sSize=1, dSize = 1,act=nn.PReLU(init=0)): #dsize refers to dilation size
        super(QConv, self).__init__()
        Cin = inChannels
        Cout  = outChannels
        Q = []
        Q.append(QuaternionConv(Cin, Cout, kernel_size = kSize, stride = sSize, dilatation = dSize, padding= dSize*(kSize-1)//2))
        if act is not None: Q.append(act)
        self.conv = nn.Sequential(*Q)
...
class QTransConv(nn.Module):
    def __init__(self, inChannels, outChannels, kSize=3, sSize=2, dSize = 1): #dsize refers to dilation size
        super(QTransConv, self).__init__()
        Cin = inChannels
        Cout  = outChannels
        self.conv = nn.Sequential(*[
            QuaternionTransposeConv(Cin, Cout, kernel_size = kSize, stride = sSize, dilatation = dSize, padding= dSize*(kSize-1)//2, output_padding=dSize*(kSize-1)//2),
            nn.PReLU(init=0)
        ])
...
class QBlock(nn.Module):
    def __init__(self, inChannels,kSize=3,dilated = [1,2,3,4]):
        ...
        self.m10 = QConv(Cin,Cout//2,kSize,sSize=1,dSize=dilated[0])
        self.m11 = QConv(Cin,Cout//2,kSize,sSize=1,dSize=dilated[1])
        self.m12 = QConv(Cin,Cout//2,kSize,sSize=1,dSize=dilated[2])
        self.m13 = QConv(Cin,Cout//2,kSize,sSize=1,dSize=dilated[3])       
        
        self.m20 = QConv(Cin*2,Cout//2,kSize,sSize=1,dSize=dilated[0]) #concat(m1,x)->m2
        self.m21 = QConv(Cin*2,Cout//2,kSize,sSize=1,dSize=dilated[1])
        
        self.s3 = QConv(2*Cin,Cout,kSize,sSize=1,dSize=1)  #concat(m2,m1) -> s3
...
'''
class QBlock3(nn.Module):
    def __init__(self, inChannels,kSize=3,dilated = [1,2,3,4]):
        super(QBlock3, self).__init__()
        Cin = inChannels
        Cout  = inChannels
        ...
        self.m20 = QConv(2*Cin,Cout//2,kSize,sSize=1,dSize=dilated[0])
        self.m21 = QConv(2*Cin,Cout//2,kSize,sSize=1,dSize=dilated[1])         
        
        self.s3 = QConv(3*Cin,Cout,kSize,sSize=1,dSize=1)  #concat(m2,m1) -> s3
    ...
    def forward(self, x):
        ...
        xm20 = self.m20(torch.cat((xm10,xm11,x),1))
        xm21 = self.m21(torch.cat((xm10,xm11,x),1))
        
        
        xs3 = self.s3(torch.cat((xm20,xm21,xm10,xm11,x),1))
        
        return xs3

class QBlock2(nn.Module):
    def __init__(self, inChannels,kSize=3,dilated = [1,2,3,4]):
        super(QBlock2, self).__init__()
        Cin = inChannels
        Cout  = inChannels
        ...
        self.m20 = QConv(Cin*2,Cout,kSize,sSize=1,dSize=dilated[0]) #concat(m1,x)->m2
    ...
    def forward(self, x):
        ...
        xs3 = self.s3(torch.cat((xm20,xm10,xm11),1))
        
        return xs3

class QBlock(nn.Module):
    def __init__(self, channel_in,kSize=3,act=nn.PReLU()):
        super(QBlock, self).__init__()

        Cin = channel_in
        Cout = channel_in
        self.conv_1 = QConv2d(3*Cin,Cout//2,kernel_size=(kSize,kSize),padding=((kSize-1)//2,(kSize-1)//2),activation=act)
        self.conv_2 = QConv2d(3*3*Cin//2,Cout//2,kernel_size=(kSize,kSize),padding=((kSize-1)//2,(kSize-1)//2),activation=act)
        self.conv_3 = QConv2d(3*Cin*2,Cout,kernel_size=(kSize,kSize),padding=((kSize-1)//2,(kSize-1)//2),activation=act)
    ...
    def forward(self, x):
        ...
        out = self.conv_2(conc)

        conc = torch.cat([conc, out], 1)

        out = self.conv_3(conc)

        out = torch.add(out, residual)

        return out
        
class MSRD(nn.Module):
    def __init__(self, inChannels,kSize=3,dilated = [1,2,4]):
        super(MSRD, self).__init__()
        Cin = inChannels
        Cout  = inChannels
        self.s1 = MSRD_Conv(Cin,Cout,kSize,dSize=1)
        
        self.m10 = MSRD_Conv(Cin,Cout,kSize,dSize=dilated[0])
        self.m11 = MSRD_Conv(Cin,Cout,kSize,dSize=dilated[1])
        self.m12 = MSRD_Conv(Cin,Cout,kSize,dSize=dilated[2])
        
        self.s2 = MSRD_Conv(3*Cin,Cout,kSize,dSize=1)
        
        self.m20 = MSRD_Conv(2*Cin,Cout,kSize,dSize=dilated[0])
        self.m21 = MSRD_Conv(2*Cin,Cout,kSize,dSize=dilated[1])
        self.m22 = MSRD_Conv(2*Cin,Cout,kSize,dSize=dilated[2])
        
        self.s3 = MSRD_Conv(3*Cin,3*Cout,kSize,dSize=1)
        
        self.s4 = MSRD_Conv(3*Cin,Cout,kSize,dSize=1)
    ...
    def forward(self, x):
        ...
        xs2 = self.s2(torch.cat((xm10,xm11,xm12),1))+x
        
        xm20 = self.m20(torch.cat((xm10,xs2),1))
        xm21 = self.m21(torch.cat((xm11,xs2),1))
        xm22 = self.m22(torch.cat((xm12,xs2),1))
        
        xs3 = self.s3(torch.cat((xm20,xm21,xm22),1))
        
        xs4 = self.s4(xs3)
        
        return x+xs4
        
class MSRD2(nn.Module):
    def __init__(self, inChannels,kSize=3,dilated = [1,2,3,4]):
        super(MSRD2, self).__init__()
        Cin = inChannels
        Cout  = inChannels
        self.s1 = MSRD_Conv(Cin,Cout,kSize,dSize=1) #s1+x-> m1
        ...
        self.s2 = MSRD_Conv(2*Cin,Cout,kSize,dSize=1) # concat(m1,s1+x) ->s2
        
        self.m20 = MSRD_Conv(Cin*3//2,Cout//2,kSize,dSize=dilated[0]) #concat(s2+x,,m1)->m2
        self.m21 = MSRD_Conv(Cin*3//2,Cout//2,kSize,dSize=dilated[1])
        
        self.s3 = MSRD_Conv(3*Cin,Cout,kSize,dSize=1)  #concat(m2,s2+x,s1+x) -> s3
    ...
    def forward(self, x):
        ...
        xs2 = self.s2(torch.cat((xm10,xm11,xm12,xm13,xs1+x),1))
        
        xm20 = self.m20(torch.cat((xm10,xm11,xs2+x),1))
        xm21 = self.m21(torch.cat((xm12,xm13,xs2+x),1))
        
        xs3 = self.s3(torch.cat((xm20,xm21,xs2+x,xs1+x),1))
        
        return x+xs3
        
class MSRD4(nn.Module):
    def __init__(self, inChannels,kSize=3,dilated = [1,2,3,4]):
        super(MSRD4, self).__init__()
        Cin = inChannels
        Cout  = inChannels
...
class MSRD5(nn.Module):
    def __init__(self, inChannels,kSize=3,dilated = [1,2,3,4]):
        super(MSRD5, self).__init__()
        Cin = inChannels
        Cout  = inChannels
        ...
        self.m20 = MSRD_Conv(2*Cin,Cout//2,kSize,dSize=dilated[0])
        self.m21 = MSRD_Conv(2*Cin,Cout//2,kSize,dSize=dilated[1])
    ...
    def forward(self, x):
        ...
        xs3 = self.s3(torch.cat((xm20,xm21,xm11,xm10,x),1)) + x
