        ...
        with torch.no_grad():
            ...
            for idx_scale, scale in enumerate(scale_list):
                ...
                for idx_img, (lr, hr, filename) in enumerate(tqdm_test):
                    ...
                    ssim_sr = np.squeeze(sr.cpu().numpy().transpose((0, 2, 3, 1)))
                    ssim_hr = np.squeeze(hr.cpu().numpy().transpose((0, 2, 3, 1)))
                    if self.args.n_colors == 1:
                        eval_ssim += ssim(ssim_sr, ssim_hr, data_range=self.args.rgb_range, gaussian_weights=True, sigma=1.5, use_sample_covariance = False)
                    else:
                        eval_ssim += ssim(ssim_sr, ssim_hr, data_range=self.args.rgb_range, multichannel=True, gaussian_weights=True, sigma=1.5, use_sample_covariance = False)
                ...
                self.ckp.write_log(
                    '\tSSIM: {:.4f}'.format(float(eval_ssim))
                )
