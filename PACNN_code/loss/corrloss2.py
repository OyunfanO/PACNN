...
class corr_loss2(torch.nn.Module):

    def __init__(self):
        super(corr_loss2, self).__init__()
    ...
    def forward(self, x, y):
        ...
        alpha = 0.5
        beta = 0.25
        corr1 = torch.norm(Qd, p=1)/torch.numel(Qd)
        corr2 = torch.norm(Qd, p=2)/torch.numel(Qd)
        l1 = F.l1_loss(x, y)
        loss = alpha*l1 + beta*(corr1+corr2)
